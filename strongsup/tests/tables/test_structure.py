...
class TestValues(object):
    ...
    def test_ensure_same_type(self):
        ...
        assert ensure_same_type({x: {(x*1.)**y for y in xrange(x)} for x in [2, 3, 5, 7]}) == 'N'
        assert ensure_same_type({x: {'fb:hello.' + str(y) for y in xrange(x)} for x in [2, 3, 5, 7]}) == 'fb:hello'
