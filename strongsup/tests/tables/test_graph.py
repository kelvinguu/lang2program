...
class KnowledgeGraphTester(object):
    __metaclass__ = ABCMeta

    def test_properties(self, graph):
        assert graph.all_rows == {'fb:row.r{}'.format(x) for x in xrange(1, self.num_rows + 1)}
        assert graph.all_columns == {'fb:row.row.{}'.format(x) for x in self.columns}
        for name, original_string in self.sample_original_strings.items():
...
class TestGraphNt0(KnowledgeGraphTester):
    ...
    join_triples = [
            ...
            ({'fb:row.r{}'.format(x) for x in xrange(1, 11) if x != 5}, 'fb:row.row.index',
                NeqInfiniteSet(5.0)),
            ({'fb:row.r{}'.format(x) for x in xrange(1, 11) if x != 5}, 'fb:row.row.year',
