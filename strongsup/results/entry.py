...
class Entry(object):
    ...
    def best(self):
        ...
        Returns:
            ...
            ResultValue
        """
        return max(self._results.iteritems(), key=operator.itemgetter(1))
    ...
    def avg(self):
        ...
        """
        return reduce(
                operator.add, self._results.values()) / len(self._results)
    ...
    def var(self):
        ...
        return reduce(operator.add, ((value - self.avg).squared()
                for value in self._results.values())) / len(self._results)
